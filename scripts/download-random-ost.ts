...
const localAudioWavPath = path.join(process.cwd(), "public", "ost.wav");
const localAudioMp3Path = path.join(process.cwd(), "public", "ost.mp3");
...
async function main() {
  ...
  const listData = await listResponse.json() as CloudinaryResponse;
  ...
  if (!audios || audios.length === 0) {
    console.error("❌ Aucune musique trouvée dans le dossier 'ost'.");
    process.exit(1);
  }

  console.log(`🎵 ${audios.length} musiques trouvées dans le dossier 'ost'`);
